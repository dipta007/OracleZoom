...
def build_vlm(vlm_lora_path):
    ...
    # Prefer flash_attention_2: the vision tower then uses varlen flash-attn (no dense [T,T]
    # mask), which is what keeps a batched eval sweep over many images from OOMing. It needs a
    # compiled flash_attn, so fall back to sdpa rather than refusing to run at all: single images
    # are fine on sdpa, and a large sweep is the only case that actually needs the varlen path.
    try:
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            name, torch_dtype="auto", device_map="auto", attn_implementation="flash_attention_2"
        )
    except (ImportError, ValueError) as e:
        print(f"#### flash-attn unavailable ({type(e).__name__}); falling back to sdpa. Fine for "
              f"a few images; install flash_attn before a large batched eval sweep.")
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
...
def resize_and_center_crop(img, size):
    """Verbatim copy of CoZ's helper (ref/coz/inference_coz.py).

    Inlined on purpose. Importing it from inference_coz pulls in that module's RAM
    tag-model stack (fairscale, timm, scipy, einops), which this pipeline never uses:
    RAM only serves CoZ's `dape` prompt baseline, and we prompt with the VLM. Four
    dependencies and a headless-OpenCV trap, for eight lines of PIL.
    """
    w, h = img.size
    scale = size / min(w, h)
...
def run(mode, gt_dir, out_dir, coz_ckpt, sd3, rec_num=4, upscale=4, process_size=512,
    ...
    if os.path.isfile(gt_dir):
        if os.path.splitext(gt_dir)[1].lower() not in (".png", ".jpg", ".jpeg"):
            raise ValueError("input must be a PNG or JPEG image")
        images = [gt_dir]
    elif os.path.isdir(gt_dir):
        images = sorted(p for e in ("*.png", "*.jpg", "*.jpeg") for p in glob.glob(f"{gt_dir}/{e}"))
    else:
        raise FileNotFoundError(f"input does not exist: {gt_dir}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--gt_dir", required=True, help="PNG/JPEG image or directory of images")
